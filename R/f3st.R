...
f3st<-function(y,x,m,p0=0.01,kmn=0,kmx=0,kex=0,mx=21,sub=T,inr=T,xinr=F,qq=-1,kexmx=100){
        covch<-double(kexmx)
        covch<-matrix(covch,nrow=1,ncol=kexmx)
        a<-f1st(y,x,p0=p0,kmn=kmn,kmx=kmx,mx=mx,kex=kex,sub=sub,inr=inr,xinr=xinr,qq=-1)
        if(a[[1]][1,1]>0){
                ali<-length(a[[1]][,1])
                ind<-(1:ali)[a[[1]][,1]>0]
                ind<-a[[1]][ind,1]
	...
	kex<-ind
                lai<-1
                if(m>0){
                        a<-f3sti(y,x,covch,ind,m,kexmx=kexmx,p0=p0,kmn=kmn,kmx=kmx,mx=mx,kex=kex,sub=sub,inr=inr,xinr=xinr,qq=-1)
                        covch<-a[[1]]
                        lai<-length(covch[,1])
                }
                if(lai>0){
