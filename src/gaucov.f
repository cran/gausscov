...
C
     ...
     $     ,minss,ss01,qq,kmn,lkx)
      ...
      double precision y(n),x(n,k),x2(n),res(n) ,pp(k+1,2),minss(n),
     $     ss01(k)
      integer ia(k),kex(lkx)
...
C
C
      subroutine genint(x,xx,n,k,kk,intx,ord,ind,ji)
      integer n,k,kk,ord,ji
      integer intx(kk,ord),ind(ord)
...
C
C
      subroutine lagg(x,n,m,i,lag,xl,y)
      integer n,m,lag,i
      double precision x(n,m),xl(n-lag,m*lag),y(n-lag)
C
C
      do 10 j=1,n-lag
         y(j)=x(lag+j,i)
